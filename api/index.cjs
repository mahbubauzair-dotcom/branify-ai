...
function resolveProviderName(modelId, ownedBy) {
...
function formatModelDisplayName(modelId) {
  ...
  return modelId.split(/[-_]/).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
}
...
function resolveSpeedLatency(model) {
  const idLower = model.id.toLowerCase();
...
function resolveCostInfo(model) {
  const idLower = model.id.toLowerCase();
  const desc = (model.description || "").toLowerCase();
...
function curateRecommendedModels(rawModels) {
  ...
  const curated = [];
  const usedModelIds = /* @__PURE__ */ new Set();
  ...
  const findModelForRole = (roleKeyword, fallbackId) => {
...
var cachedRawModels = null;
var cachedCuratedModels = null;
var cachedRoutingConfig = null;
var lastFetchTimestamp = 0;
var CACHE_TTL_MS = 3 * 60 * 1e3;
var getVectorEngineApiKey = () => {
  return process.env.VECTOR_ENGINE_API_KEY || "";
};
var getVectorEngineBaseUrl = () => {
  return (process.env.VECTOR_ENGINE_BASE_URL || "https://api.vectorengine.ai").replace(/\/+$/, "");
};
var getGooglePlacesApiKey = () => {
  return process.env.GOOGLE_PLACES_API_KEY || process.env.GOOGLE_MAPS_API_KEY || "";
};
async function fetchVectorEngineModels() {
  const now = Date.now();
...
function resolveModelForTask(task, routingConfig) {
...
var AUTHORIZED_OWNER_EMAIL = "mahbubauzair@gmail.com";
app.post("/api/auth/owner-login", (req, res) => {
  ...
  const ttlMs = rememberMe ? 7 * 24 * 60 * 60 * 1e3 : 24 * 60 * 60 * 1e3;
...
app.post("/api/vectorengine/chat", async (req, res) => {
  ...
  const apiKey = getVectorEngineApiKey();
  const baseUrl = getVectorEngineBaseUrl();
  const makeCall = async (modelToUse) => {
    const payload = {
      model: modelToUse,
      messages,
    ...
    try {
      ...
      if (gRes.ok) {
        ...
        if (gData.status === "OK" && Array.isArray(gData.results)) {
          const formatted = gData.results.map((place) => ({
            id: place.place_id,
            ...
            opportunityScore: !place.website ? Math.min(99, Math.max(70, Math.round((place.rating || 4) * 18 + (place.user_ratings_total > 20 ? 10 : 0)))) : 45
          }));
          return res.json({
            success: true,
