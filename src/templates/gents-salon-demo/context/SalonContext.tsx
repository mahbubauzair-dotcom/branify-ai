...
export const SalonProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [config, setConfig] = useState<TemplateConfig>(() => {
    try {
      const saved = localStorage.getItem('royalcrown_salon_config');
  ...
  const [adminUser, setAdminUser] = useState<AdminUser | null>(() => {
    try {
      const saved = localStorage.getItem('royalcrown_admin_session');
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    return null;
  });
    ...
    const loadFromSupabase = async () => {
      ...
      try {
        ...
        if (salonData) {
          ...
          localStorage.setItem('royalcrown_salon_config', JSON.stringify(fetchedConfig));
        }
        ...
        if (sessionData?.session?.user) {
          ...
          if (adminRecord) {
            ...
            localStorage.setItem('royalcrown_admin_session', JSON.stringify(adminObj));
            ...
            setAdminUser(null);
            localStorage.removeItem('royalcrown_admin_session');
          }
        }
  ...
  const persistConfig = (c: TemplateConfig) => {
    setConfig(c);
    try { localStorage.setItem('royalcrown_salon_config', JSON.stringify(c)); } catch (e) {}
  };
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (authUser) {
          ...
          localStorage.setItem('royalcrown_admin_session', JSON.stringify(userObj));
  ...
  const logoutAdmin = async () => {
    ...
    setAdminUser(null);
    try { localStorage.removeItem('royalcrown_admin_session'); } catch (e) {}
  };
