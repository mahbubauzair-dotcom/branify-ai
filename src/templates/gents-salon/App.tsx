...
function MainAppContent() {
  ...
  const [isAdminPath, setIsAdminPath] = useState<boolean>(() => {
    return window.location.pathname.startsWith('/template/gents-salon/admin');
  });

  const [adminRoute, setAdminRoute] = useState<AdminRoute>(() => {
    const path = window.location.pathname as AdminRoute;
    if (path.startsWith('/template/gents-salon/admin')) {
      return path;
    }
    return '/template/gents-salon/admin';
  });
  ...
  const navigatePublicPage = (page: NavigationPage) => {
    setCurrentPage(page);
    setIsAdminPath(false);
    window.history.pushState({}, '', '/template/gents-salon');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  // IF ADMIN ROUTE
  if (isAdminPath) {
    if (!isAuthenticated || adminRoute === '/template/gents-salon/admin/login') {
      return (
        <AdminLogin
          onSuccess={() => navigateAdmin('/template/gents-salon/admin')}
    ...
    return (
      <AdminLayout
        ...
        onNavigatePublic={() => navigatePublicPage('home')}
      >
        {adminRoute === '/template/gents-salon/admin' && (
          <AdminDashboard onNavigate={(route) => navigateAdmin(route)} />
        )}
        {adminRoute === '/template/gents-salon/admin/appointments' && (
          <AdminAppointments />
        )}
        {adminRoute === '/template/gents-salon/admin/services' && (
          <AdminServices />
        )}
        {adminRoute === '/template/gents-salon/admin/offers' && (
          <AdminOffers />
        )}
        {adminRoute === '/template/gents-salon/admin/gallery' && (
          <AdminGallery />
        )}
        {adminRoute === '/template/gents-salon/admin/reviews' && (
          <AdminReviews />
        )}
        {adminRoute === '/template/gents-salon/admin/settings' && (
          <AdminSettings />
        )}
      </AdminLayout>
    );
