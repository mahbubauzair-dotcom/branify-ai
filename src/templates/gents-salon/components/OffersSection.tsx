      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {activeOffers.map((offer) => (
            <div
              key={offer.id}
              className="bg-stone-950 rounded-2xl border border-stone-800 hover:border-amber-500/50 p-6 flex flex-col justify-between relative transition-all group"
            >
              <div>
                {/* Badge */}
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[10px] uppercase tracking-wider font-bold bg-amber-500/10 text-amber-400 border border-amber-500/30 px-2.5 py-1 rounded-full">
                    {offer.badge}
                  </span>
